...
```
段间倍率 = 共享曲线 × 爽点 × 被动(1.15^新增数) × 新分支(间接)
```
...
|----|---------|------|----------|---------|---------|---------|
| 1→10 | ×2.16 | ×1.20 | 1(Lv6) | ×1.15 | **×2.98** | ×2.0~×4.0 |
| 10→20 | ×2.31 | ×1.25 | 1(Lv14) | ×1.15 | **×3.32** | ×2.0~×4.0 |
| 20→30 | ×1.61 | ×1.33 | 3(Lv22/26/30) | ×1.52 | **×3.26** | ×2.0~×4.0 |

> 被动乘区用 1.15^(新增被动数) 统一计算。
...
| 10 | ×2.16 | ×1.39 | ×1.15(1个) | **×3.5** |
| 20 | ×5.00 | ×1.80 | ×1.32(2个) | **×11.9** |
| 30 | ×8.04 | ×3.36 | ×2.01(5个) | **×54.3** |

> 注意：含被动后 Lv30 局外总倍率从 ×27 上升到 ×54.3。
...
|--------|-------|---------|-------------|--------|
| Lv1  | 20 | ×86.7  | ×1.0   | **×87** |
| Lv10 | 25 | ×264.7 | ×3.5   | **×926** |
| Lv20 | 26 | ×330.9 | ×11.9  | **×3,938** |
| Lv30 | 26 | ×330.9 | ×54.3  | **×17,968** |
...
例：冰爆弹 无分支Lv1 DPS = 1.0，n=1.25，26点，5被动
  ...
  → Lv30满分支预测 = 1.0 × 330.9 × 54.3 = 17,968
```
