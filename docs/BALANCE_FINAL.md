...
| 指标 | 目标值 | 说明 |
|------|--------|------|
| 等效 n | **1.25** | 统一设计值，每点 +25% 等效DPS |
| 25点总乘数 | ×264.7 | 1.25^25 |
| 26点总乘数 | ×330.9 | 1.25^26 |
...
> n=1.25 是统一设计值。n<1.20 后期无感，n>1.35 前期太弱后期太爆。
...
> ×1.25 是**设计标定值**，用于模型预测。
> 实际被动效果应围绕此值设计：允许个别被动偏高/偏低，但整体等效应接近。
> **红线：单个被动禁止超过 ×1.50**（见3.6）。
> 与分支等效n=1.25保持一致，简化模型。
...
```
段间倍率 = 共享曲线 × 爽点 × 被动(1.25^新增数) × 新分支(间接)
```

以冰爆弹为例（被动解锁：Lv6/14/22/26/30）：

| 段 | 共享曲线 | 爽点 | 新增被动数 | 被动乘区 | 目标合计 | 容忍范围 |
|----|---------|------|----------|---------|---------|---------|
| 1→10 | ×2.16 | ×1.20 | 1(Lv6) | ×1.25 | **×3.24** | ×2.0~×4.0 |
| 10→20 | ×2.31 | ×1.25 | 1(Lv14) | ×1.25 | **×3.61** | ×2.0~×4.0 |
| 20→30 | ×1.61 | ×1.33 | 3(Lv22/26/30) | ×1.95 | **×4.17** | ×2.0~×4.0 |

> 被动乘区用 1.25^(新增被动数) 统一计算。
> 20→30 共享曲线最弱(×1.61)，但 3 个被动(×1.52)补偿。
...
局外成长 = shopDmgMult(lv) × sweetSpotGrowth(lv) × passiveBonus(lv)
局内成长 = n ^ totalPoints

其中：
  passiveBonus(lv) = 1.25 ^ 已解锁被动数
```

**局外成长倍率**（基于每10级×3目标）：

以冰爆弹为例（被动：Lv6/14/22/26/30 共5个）：

| ShopLv | 共享曲线 | 爽点 | 被动(1.15^n) | 局外总倍率(vs Lv1) |
|--------|---------|------|-------------|-------------------|
| 1  | ×1.00 | ×1.00 | ×1.00(0个) | **×1** |
| 10 | ×2.16 | ×1.39 | ×1.25(1个) | **×3.8** |
| 20 | ×5.00 | ×1.80 | ×1.56(2个) | **×14.0** |
| 30 | ×8.04 | ×3.36 | ×3.05(5个) | **×82.4** |

> 注意：含被动后 Lv30 局外总倍率从 ×27 上升到 ×82.4。
> 不同武器被动数/解锁节点不同，需逐武器计算。
...
| ShopLv | 分支点 | 分支乘数 | 局外(含被动) | 总倍率 |
|--------|-------|---------|-------------|--------|
| Lv1  | 20 | ×86.7  | ×1.0   | **×87** |
| Lv10 | 25 | ×264.7 | ×3.8   | **×1,006** |
| Lv20 | 26 | ×330.9 | ×14.0  | **×4,633** |
| Lv30 | 26 | ×330.9 | ×82.4  | **×27,266** |

> 分支解锁：Lv5 +chain(2), Lv10 +splitBomb(3), Lv18 +giant(1)
> 被动解锁：Lv6(1个), Lv14(2个), Lv22(3个), Lv26(4个), Lv30(5个)
> 不同武器需按各自分支/被动数单独计算
...
例：冰爆弹 无分支Lv1 DPS = 1.0，n=1.25，26点，5被动
  → Lv1满分支预测 = 1.0 × 86.7 × 1.0 = 87
  → Lv30满分支预测 = 1.0 × 330.9 × 82.4 = 27,266
```
