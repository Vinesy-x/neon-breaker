...
> 这是整个数值框架的**核心结论**。
> 后续沙盒测试的实测值与此对比，验证模型是否准确，指导平衡调参。

**预测条件**：baseAttack=1，满分支（所有非shopGated分支点满），指定商店等级。

| 武器 | 分支点 | 分支总乘数 | 等效n | Lv1 | Lv10 | Lv20 | Lv30 |
|------|--------|-----------|-------|-----|------|------|------|
| 冰爆弹 | 26 | ×411 | 1.26 | 4,933 | 12,787 | 36,998 | 79,324 |
| 闪电链 | 25 | ×562 | 1.29 | 3,993 | 14,376 | 46,588 | 96,318 |
| 穿甲弹 | 24 | ×838~299* | 1.25* | 25,388 | 82,256 | 253,876 | 510,290 |
| 轰炸机 | 24 | ×648 | 1.31 | 5,184 | 14,930 | 51,840 | 166,717 |

> *穿甲弹分支乘数随等级变化（salvo分支+3叠加在商店齐射上，低等级放大效应更大）

**段间比（预测）**：

| 武器 | 1→10 | 10→20 | 20→30 |
|------|------|-------|-------|
| 冰爆弹 | ×2.59 | ×2.89 | ×2.14 ⚠️ |
| 闪电链 | ×3.60 | ×3.24 | ×2.07 |
| 穿甲弹 | ×3.24 | ×3.09 | ×2.01 ⚠️ |
| 轰炸机 | ×2.88 | ×3.47 | ×3.22 ✅ |

**Lv30 最强/最弱比：×6.4（目标 <3x）** 🔴

**核心问题**：穿甲弹在所有等级都是其他武器的 5~6 倍，是当前最大的平衡问题。

**模型置信度**：
- 闪电链：Lv1 预测 3,993 vs 实测 4,098 → **误差 -2.6%** ✅ 模型可信
- 其余武器：待沙盒验证
...
| 段 | 共享曲线 | 爽点目标贡献 | 合计目标 | 容忍范围 |
|----|---------|------------|---------|---------|
| 1→10 | ×2.16 | ×1.39 | ×3.0 | ×2.0~×4.0 |
| 10→20 | ×2.31 | ×1.30 | ×3.0 | ×2.0~×4.0 |
| 20→30 | ×1.61 | ×1.86 | ×3.0 | ×2.0~×4.0 |

> 20→30 段爽点需贡献更多(×1.86)，因为共享曲线在高等级趋于平缓。
> CD型爽点天然适合补偿这一段；数量型爽点需注意线性增长的纯乘区问题。
...
10. 铺初始砖块            → ceil(targetAlive/6/2) 行
```
...
ratio < 0.5:   生双行 + HP ×1.12
ratio < 0.8:   生一行 + HP ×1.05
ratio 0.8~1.1: 正常生一行
ratio > 1.1:   停止生砖
```

稳定检测：ratio 连续5次在 0.7~1.3 → 水位稳定 → 预热结束。
