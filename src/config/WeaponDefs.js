...
const WEAPON_TREES = {
  ...
  drone: {
    ...
    branches: {
      ...
      count:      { name: '阵列', desc: '+1台无人机(阵型升级)', max: 3, requires: null },
      speed:      { name: '机动', desc: '+30%阵型移动速度', max: 3, requires: null },
      width:      { name: '光束', desc: '+40%激光判定范围', max: 2, requires: { damage: 2 } },
      deploy:     { name: '部署', desc: '+25%巡逻区域', max: 2, requires: { speed: 2 } },
      rotate:     { name: '旋阵', desc: '阵型旋转切割加速', max: 2, requires: { count: 2 } },
      overcharge: { name: '过载', desc: '激光交叉点伤害×2', max: 1, requires: { count: 2, damage: 3 } },
      pulse:      { name: '脉冲', desc: '每5秒全阵型AOE爆发', max: 1, requires: { damage: 4, rotate: 2 } },
    },
  },
  spinBlade: {
